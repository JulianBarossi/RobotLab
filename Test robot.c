...
#pragma config(Motor,  port6,  robotArm,       tmotorServoStandard, openLoop) 
	...
	while (true) {
		...
		if (start) {
			while (start) {
				...
				if (SensorValue[button] == 1 && buttonFirstPressed) {
					start = false;
					motor[leftMotor] = 0;
					motor[rightMotor] = 0;
					wait1Msec(200);
				...
				if (state == 1) {
					...
					while (SensorValue[limitSwitch] == 1){
						...
						Find_max();
						Move();
						if (SensorValue[limitSwitch] == 0) { // Detect limit switch press
							//wait1MSec(500);
							motor[leftMotor] = 0;
							motor[rightMotor] = 0;
						}
