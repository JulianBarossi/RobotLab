...
task main(){
	...
	*/
	...
	while(true){
		...
		if (state == 1) //finds erd beacon and goes to it
		{
			ReadPD();
			Find_max();
			Move();
		...
		if (state == 2) //presses red beacon
		{
			...
			if (SensorValue[digital10] - 1000 < 200)
			{
				state++;
			}
		}

		if (state == 3) //finds green beacon
		{
			motor[port1] = -100;
			motor[port10] = -100;
			wait1Msec(2000);
		...
		if (state == 4) //presses green beacon
		{
			motor[port5] = 100;
			wait1Msec(5000);
			motor[port5] = 0;

			if (SensorValue[digital10] - 10000 < 200)
			{
				state++;
			}
